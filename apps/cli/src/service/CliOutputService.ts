...
import { exitCode, ICliErrorService, ICliRuntimeService } from '../contract/index'
import type {
  CliContext,
  CommandPresentation,
  ICliOutputService,
...
export class CliOutputService implements ICliOutputService {
  ...
  async runCommand(
    ...
    try {
      ...
      presentation = { exitCode: output.ok ? exitCode.success : exitCode.failure, output }
    } catch (error) {
  ...
  async runRepoMirror(
    ...
    try {
      ...
      if (context.agent) process.stderr.write(`${presentation.output.warnings?.[0] ?? ''}\n`)
    }
    ...
    if (context.format === 'json') return data
    return context.ok(data, { cta: { commands: [...(options.nextCommands ?? [])] } })
  }
