...
export class LocalGitRepositoryGateway implements GitRepositoryGateway {
  ...
  async inspect(
    ...
    if (pathState === 'missing') return repositoryResult(repository, 'missing')
    if (pathState === 'not-directory') return repositoryResult(repository, 'not-git')
    ...
    try {
      ...
      if (!isSuccessful(worktree) || worktree.stdout.trim() !== 'true') {
        return repositoryResult(repository, 'not-git')
      }
      ...
      if (comparison === null || !comparison.isSameRepository) {
        return repositoryResult(repository, 'wrong-repo', actual, flags)
      }
      if (facts.branch === null) return repositoryResult(repository, 'detached', actual, flags)
      if (facts.branch !== repository.branch)
        return repositoryResult(repository, 'wrong-branch', actual, flags)
      if (facts.upstream === null) return repositoryResult(repository, 'local-only', actual, flags)

      const remoteRef = remoteTrackingRef(facts.upstream)
      if (remoteRef === null) return repositoryResult(repository, 'wrong-upstream', actual, flags)
      ...
      if (!upstreamReference.exists)
        return repositoryResult(repository, 'upstream-gone', actual, flags)
      if (facts.upstream !== `origin/${repository.branch}`) {
        return repositoryResult(repository, 'wrong-upstream', actual, flags)
      }
  ...
  async prepareBranch(
    ...
    if (remoteBranch.exists) {
      ...
      commands.push(command)
      return { commands, kind: isSuccessful(command) ? 'tracked-remote' : 'failed' }
    }

    const fromRef = remoteTrackingRef(branch.from)
    if (fromRef === null) return { commands, kind: 'failed' }
    const baseBranch = await this.hasRef(path, fromRef, options)
    commands.push(baseBranch.command)
    if (isLookupFailure(baseBranch) || !baseBranch.exists) return { commands, kind: 'failed' }
    ...
    commands.push(command)
    return { commands, kind: isSuccessful(command) ? 'created-from' : 'failed' }
  }
  ...
  async probeRemote(url: string, options: GitOperationOptions = {}): Promise<GitCommandResult> {
    return this.#runner.run(['ls-remote', '--symref', url, 'HEAD'], {
      ...
      stdio: options.interactive === true ? 'interactive-pipe' : 'pipe',
    })
  }
  ...
  async readCheckout(path: string, options: GitOperationOptions): Promise<CheckoutInspection> {
...
export function createGitRepositoryGateway(runner?: GitCommandRunner): GitRepositoryGateway {
  return new LocalGitRepositoryGateway(runner)
}
...
type PathState = 'directory' | 'missing' | 'not-directory' | 'unavailable'

async function inspectPath(path: string): Promise<PathState> {
  try {
    const metadata = await stat(path)
    return metadata.isDirectory() ? 'directory' : 'not-directory'
  } catch (error: unknown) {
  ...
  return {
    ...
    ...(inheritStdio
      ? {
          interactive: true,
          stdio:
            options.onOutput === undefined ? ('inherit' as const) : ('interactive-pipe' as const),
        }
      : {}),
  }
}
...
function repositoryFlags(
  ...
  return [
    ...(checkoutDifferent ? (['checkout-different'] as const) : []),
    ...(dirty ? (['dirty'] as const) : []),
    ...(urlDifferent ? (['url-different'] as const) : []),
  ]
}
...
function stateFromAheadBehind(
  ...
  if (counts.ahead === null || counts.behind === null) return 'synced-local'
