...
} from '../../application/ports/repo-mirror-logger.js'
...
export class PinoRepoMirrorLogger implements RepoMirrorLogger {
  readonly #paths: GitsPaths
  readonly #retention: RepoMirrorLogRetentionPolicy

  constructor(paths: GitsPaths, retention: Partial<RepoMirrorLogRetentionPolicy> = {}) {
    this.#paths = paths
    this.#retention = { ...defaultRepoMirrorLogRetentionPolicy, ...retention }
  }

  async start(name: string, source: 'manual' | 'scheduler'): Promise<RepoMirrorLogSession> {
  ...
  logDirectory(name: string): string {
...
class PinoLogSession implements RepoMirrorLogSession {
  ...
  async finish(
    ...
    this.#logger[status === 'failed' ? 'error' : 'info'](
...
function isRunState(value: unknown): value is RepoMirrorRunState {
  return (
    ...
    'source' in value &&
    (value.source === 'manual' || value.source === 'scheduler') &&
    'status' in value &&
