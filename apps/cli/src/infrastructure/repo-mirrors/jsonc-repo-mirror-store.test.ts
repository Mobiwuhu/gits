...
import { resolveGitsPaths } from './gits-paths.js'
import { JsoncRepoMirrorStore, parseRepoMirrorConfiguration } from './jsonc-repo-mirror-store.js'

describe('JsoncRepoMirrorStore', () => {
  it('preserves comments and unrelated machine settings while updating managed fields', async () => {
    ...
    const paths = resolveGitsPaths({ environment: { GITS_HOME: resolve(root, '.gits') } })
    try {
...
`,
      ...
      const store = new JsoncRepoMirrorStore(paths)
      await store.save({
        repoMirrors: [
          {
            name: 'api',
