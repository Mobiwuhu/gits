...
import type { RepoMirrorStore } from '../../application/ports/repo-mirror-store.js'
...
import { ensureGitsLayout, type GitsPaths } from './gits-paths.js'
...
const initialContent = `{
  // Machine-local Git object mirrors managed by gits.
  "version": 1,
  "repoMirrorsSettings": {
...
export class JsoncRepoMirrorStore implements RepoMirrorStore {
  readonly #paths: GitsPaths

  constructor(paths: GitsPaths) {
    this.#paths = paths
  }
  ...
  async load(): Promise<RepoMirrorConfiguration> {
    let content: string
    try {
      content = await readFile(this.#paths.config, 'utf8')
    } catch (error) {
      if (hasCode(error, 'ENOENT')) return defaultConfiguration
      const message = error instanceof Error ? error.message : String(error)
      throw new RepoMirrorConfigurationError(`Cannot read ${this.#paths.config}: ${message}`)
    }
  ...
  async save(configuration: RepoMirrorConfiguration): Promise<void> {
    await ensureGitsLayout(this.#paths)
    ...
    content = updateProperty(content, ['repoMirrorsSettings'], configuration.repoMirrorsSettings)
    content = updateProperty(content, ['repoMirrors'], configuration.repoMirrors)
    await writeFileAtomically(this.#paths.config, content)
  }
}
...
export function parseRepoMirrorConfiguration(
  ...
  if (!isObject(parsed)) throw new RepoMirrorConfigurationError(`${source} must contain an object.`)
  ...
  const maxConcurrentFetches = parseMaximumConcurrency(parsed.repoMirrorsSettings, issues)
...
function parseMaximumConcurrency(value: unknown, issues: string[]): number {
  ...
  if (maximum === undefined) return 4
  if (!Number.isInteger(maximum) || typeof maximum !== 'number' || maximum < 1 || maximum > 32) {
    issues.push('repoMirrorsSettings.maxConcurrentFetches must be an integer from 1 to 32.')
    return 4
  }
  return maximum
}

function parseMirrors(value: unknown, issues: string[]): readonly RepoMirrorDefinition[] {
  ...
  for (const [index, item] of value.entries()) {
    ...
    const schedule = parseSchedule(item.schedule, `${location}.schedule`, issues)
    if (name === undefined || urls === undefined) continue
    if (names.has(name)) {
      issues.push(`Duplicate repo mirror name '${name}'.`)
      continue
    }
    names.add(name)
    mirrors.push({ name, urls, ...(schedule === undefined ? {} : { schedule }) })
  }
  return mirrors
}

function parseName(value: unknown, location: string, issues: string[]): string | undefined {
...
function parseSchedule(
  ...
  if (!isObject(value) || typeof value.cron !== 'string' || value.cron.trim().length === 0) {
...
function unsafeUrlIssue(value: string): string | null {
  if (value.includes('\0') || /[\r\n]/u.test(value)) return 'cannot contain NUL or newlines.'
  if (value.startsWith('-')) return 'cannot begin with a hyphen.'
