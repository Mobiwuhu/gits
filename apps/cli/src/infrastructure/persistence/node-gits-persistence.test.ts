...
import type { SystemCommandResult, SystemCommandRunner } from '../process/system-command-runner.js'
import { ensureGitsLayout, resolveGitsPaths } from '../repo-mirrors/gits-paths.js'
import { NativeRepoMirrorScheduler } from '../scheduler/native-repo-mirror-scheduler.js'
import { gitsHomePersistenceRegistry } from './gits-persistence-registry.js'
import { NodeGitsPersistence } from './node-gits-persistence.js'

class SuccessfulCommandRunner implements SystemCommandRunner {
  readonly calls: { readonly args: readonly string[]; readonly executable: string }[] = []

  async run(executable: string, args: readonly string[]): Promise<SystemCommandResult> {
...
describe('gits persistence registry and uninstall cleanup', () => {
  it('derives every GITS_HOME path and directory from the central registry', async () => {
    ...
    try {
      ...
      const paths = resolveGitsPaths({ environment: { GITS_HOME: home } })
      assert.deepEqual(
  ...
  it('removes owned LaunchAgents and the data root but preserves unrelated files', async () => {
    ...
    try {
      ...
      const paths = resolveGitsPaths({ environment: { GITS_HOME: gitsHome }, userHome })
      const runner = new SuccessfulCommandRunner()
      const scheduler = new NativeRepoMirrorScheduler(paths, {
      ...
      const persistence = new NodeGitsPersistence(paths, {
  ...
  it('refuses to recursively remove the user home even when configured as GITS_HOME', async () => {
    ...
    try {
      const userHome = resolve(root, 'user-home')
      const paths = resolveGitsPaths({ environment: { GITS_HOME: userHome }, userHome })
      await ensureGitsLayout(paths)
  ...
  it('removes owned systemd user units and reloads the user manager', async () => {
    ...
    try {
      ...
      const paths = resolveGitsPaths({ environment, userHome })
      const runner = new SuccessfulCommandRunner()
      const scheduler = new NativeRepoMirrorScheduler(paths, {
        environment,
