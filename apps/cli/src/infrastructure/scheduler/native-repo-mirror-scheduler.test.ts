...
import { NativeRepoMirrorScheduler } from './native-repo-mirror-scheduler.js'
...
} from './stable-runner-installer.js'
...
describe('native repo mirror scheduler', () => {
  ...
  it('installs an executable stable runner that delegates without a shell command string', async () => {
    ...
    try {
      ...
      await import('../filesystem/atomic-file.js').then(({ writeFileAtomically }) =>
        writeFileAtomically(
          cliEntry,
          'process.stdout.write(JSON.stringify(process.argv.slice(2)))\n',
        ),
      )
      const installer = new StableRunnerInstaller(paths, cliEntry, [])
