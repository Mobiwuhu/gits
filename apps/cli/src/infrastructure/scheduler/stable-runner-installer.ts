...
import { gitsManagedArtifactRegistry } from '../persistence/gits-persistence-registry.js'
...
export class StableRunnerInstaller {
  ...
  async install(): Promise<string> {
    ...
    await writeFileAtomically(path, content, 0o700)
  ...
  invocation(name: string): RepoMirrorScheduledInvocation {
    ...
    return {
      arguments: ['repo-mirrors', 'fetch', name, '--source', 'scheduler', '--json'],
      environment: {
