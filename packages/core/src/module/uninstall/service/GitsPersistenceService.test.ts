...
void describe('gits persistence registry and uninstall cleanup', () => {
  void it('derives every GITS_HOME path and directory from the central registry', async () => {
    const root = await mkdtemp(
      resolve(tmpdir(), 'gits-persistence-registry-test-')
    )
    try {
      ...
      await Promise.all(
        Object.entries(gitsHomePersistenceRegistry)
          ...
          .map(async ([key]) =>
            access(paths[key as keyof typeof gitsHomePersistenceRegistry])
          )
      )
    } finally {
  ...
  void it('removes owned LaunchAgents and the data root but preserves unrelated files', async () => {
    const root = await mkdtemp(
      resolve(tmpdir(), 'gits-persistence-launchd-test-')
    )
    try {
      ...
      await writeFile(
        resolve(gitsHome, 'future-cache'),
        'registered by a future feature\n'
      )
      ...
      assert.ok(inventory.some((target) => target.path === projectionPath))
      assert.ok(
        inventory.some(
          (target) => target.kind === GitsPersistenceTargetKind.Unregistered
        )
      )
      ...
      await assert.rejects(async () => access(projectionPath))
      assert.match(await readFile(unrelated, 'utf-8'), /<plist>/u)
      assert.ok(
        runner.calls.some(
          (call) =>
            call.executable === '/bin/launchctl' && call.args[0] === 'bootout'
        )
      )
    } finally {
      await rm(root, { force: true, recursive: true })
  ...
  void it('refuses to recursively remove the user home even when configured as GITS_HOME', async () => {
    const root = await mkdtemp(
      resolve(tmpdir(), 'gits-persistence-safety-test-')
    )
    try {
      const userHome = resolve(root, 'user-home')
      const paths = new GitsPathService({
        environment: { GITS_HOME: userHome },
        userHome,
      })
      await paths.ensureLayout()
      await writeFile(resolve(userHome, 'keep-me'), 'user data\n')

      const persistence = new GitsPersistenceService(
        paths,
      ...
      await assert.rejects(async () => persistence.purge(), {
        code: 'uninstall-safety',
      })
      assert.equal(
        await readFile(resolve(userHome, 'keep-me'), 'utf-8'),
        'user data\n'
      )
    } finally {
      await rm(root, { force: true, recursive: true })
  ...
  void it('removes owned systemd user units and reloads the user manager', async () => {
    const root = await mkdtemp(
      resolve(tmpdir(), 'gits-persistence-systemd-test-')
    )
    try {
      const userHome = resolve(root, 'user-home')
      const gitsHome = resolve(root, 'gits-home')
      ...
      const { projectionPath } = observation
      assert.ok(projectionPath !== null)
      const enablementDirectory = resolve(
        xdgConfig,
        'systemd/user/timers.target.wants'
      )
      const enablement = resolve(enablementDirectory, basename(projectionPath))
      await mkdir(enablementDirectory, { recursive: true })
      await symlink(`../${basename(projectionPath)}`, enablement)
      ...
      const inventory = await persistence.inspect()
      const units = inventory.filter((target) =>
        target.id.startsWith('systemd')
      )
      ...
      await assert.rejects(async () => access(gitsHome))
      await Promise.all(
        units.map(async (unit) => assert.rejects(async () => access(unit.path)))
      )
      assert.ok(
        runner.calls.some(
          (call) =>
            call.executable === 'systemctl' && call.args.includes('disable')
        )
      )
      assert.ok(
        runner.calls.some(
