...
export class SwitchTaskService implements ISwitchTaskService {
  ...
  async execute(input: SwitchTaskInput): Promise<CommandOutput> {
    ...
    const fetchCandidates = plans
      .filter(
        (plan) =>
          plan.needsFetch &&
          (input.stash || !plan.initial.flags.includes(RepositoryFlag.Dirty))
      )
      .map((plan) => plan.repository)
    const preflightFailures = await preflightRemotes(
      this.git,
      fetchCandidates,
      {
    ...
    const fetched = await this.fetchMissingBranches(
      fetchCandidates,
      input,
      preflightFailures
    )
    const fetchFailures = new Map(
      fetched.map((result) => [result.name, result])
    )
    ...
    return summary.results.map((entry) => {
      ...
      if (entry.status === ConcurrentRunStatus.NotRun) {
        return withCommandError(
          ...
          commandError(
            'interrupted',
            'Repository was not switched before interruption.'
          ),
    ...
    return Promise.all(
      repositories.map(async (repository) => {
        ...
        const plannedInitial =
          localBranch.command.exitCode === 0 ||
          localBranch.command.exitCode === 1
            ? initial
    ...
    return summary.results.map((entry) => {
      ...
      if (entry.status === ConcurrentRunStatus.NotRun) {
        return withCommandError(
          ...
          commandError(
            'interrupted',
            'Fetch was not started before interruption.'
          ),
          RepositoryActionResult.NotRun
        )
      }
  ...
  private async switchPlannedRepository(
    ...
    if (!canSwitch(initial)) {
      return unavailableForSwitch(initial)
    }
    ...
    if (fetchResult?.result === RepositoryActionResult.Failed) {
      return fetchResult
    }
    ...
    if (dirty && !input.stash) {
      return withCommandError(
        ...
        commandError(
          'dirty-worktree',
          'Use --stash to save uncommitted changes before switching.'
        )
      )
    }
    if (dirty) {
      ...
      if (stashed.reference !== null) {
        updateProgress(
          input,
          task,
          repository,
          `stashed as ${stashed.reference}`
        )
        ...
        return withCommandError(
          ...
          command
            ...
            : commandError(
                ...
                'Could not switch to task branch.'
              )
        )
      }
    }
    ...
    if (checkoutChanges || (branchChanges && repository.checkout !== null)) {
      ...
      if (validationError !== null) {
        return withCommandError(initial, validationError)
      }
