...
export async function loadTaskConfiguration(
  ...
  if (placeholders.length > 0 && input.allowIncomplete !== true) {
    throw new IncompleteConfigurationError(placeholders)
  }

  const repositoriesForReferenceValidation =
    input.allowIncomplete === true
      ? configuration.repositories.filter(
          (repository) => !isRepositoryIncomplete(repository)
        )
      : configuration.repositories
  await validateBranchNames(
    repositoriesForReferenceValidation,
    branchValidator,
    input.signal
  )
...
async function validateBranchNames(
  ...
  const checks = await Promise.all(
    uniqueCandidates.map(async (branch) => ({
      branch,
      valid: isValidReference(
        await branchValidator.checkRefFormat(branch, options)
      ),
    }))
  )
  ...
  if (invalid.length > 0) {
    throw new ConfigurationError(
      `Invalid Git branch names: ${invalid.join(', ')}`
    )
  }
}
