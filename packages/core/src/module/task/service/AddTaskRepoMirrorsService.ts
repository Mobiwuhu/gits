...
import type {
  ...
  AddTaskRepoMirrorsInput,
  AddTaskRepoMirrorsOutput,
} from '../../../contract/index'
import { selectRepositories } from './taskSelection'

export class AddTaskRepoMirrorsService implements IAddTaskRepoMirrorsService {
  constructor(
    @Inject(IAddRepoMirrorService)
    private readonly addRepoMirror: IAddRepoMirrorService,
    @Inject(ITaskConfigurationService)
    private readonly configuration: ITaskConfigurationService
  ) {}

  async execute(
    input: AddTaskRepoMirrorsInput
  ): Promise<AddTaskRepoMirrorsOutput> {
    const loaded = await this.configuration.load(input.root)
    const urls = selectRepositories(
