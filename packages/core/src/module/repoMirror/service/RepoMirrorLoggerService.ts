...
import {
  IFileSystemService,
  IGitsPathService,
  type IRepoMirrorLoggerService,
  RepoMirrorInvocationSource,
  RepoMirrorLastRunStatus,
  type RepoMirrorLogSession,
  type RepoMirrorRunState,
} from '../../../contract/index'
...
export class RepoMirrorLoggerService implements IRepoMirrorLoggerService {
  ...
  async start(name: string, source: RepoMirrorInvocationSource): Promise<RepoMirrorLogSession> {
    ...
    const destination = pino.destination({ dest: activePath, mkdir: true, mode: 0o600, sync: true })
    const logger = pino(
      { base: { mirror: name, source }, timestamp: pino.stdTimeFunctions.isoTime },
      destination,
    )
  ...
  async readLastRun(name: string): Promise<RepoMirrorRunState | null> {
    try {
      const value: unknown = JSON.parse(await readFile(this.statePath(name), 'utf8'))
  ...
  async readLines(name: string, maximum: number): Promise<readonly string[]> {
    ...
    const contents = await Promise.all(
      entries.map(async (entry): Promise<string | null> => {
        try {
          return await readFile(resolve(directory, entry), 'utf8')
        } catch (error) {
          if (hasCode(error, 'ENOENT')) return null
          throw error
        }
    ...
    for (const content of contents) {
      ...
      lines.unshift(...fileLines.slice(Math.max(0, fileLines.length - (maximum - lines.length))))
      if (lines.length >= maximum) return lines.slice(-maximum)
    }
...
class PinoLogSession implements RepoMirrorLogSession {
  ...
  async finish(
    ...
    const error = options.error === undefined ? undefined : sanitize(options.error)
    this.#logger[status === RepoMirrorLastRunStatus.Failed ? 'error' : 'info'](
      { durationMs, event: 'finish', ...(error === undefined ? {} : { error }), status },
...
async function retainCompletedLogs(
  ...
  const files = (await listCompletedLogs(directory)).toSorted(compareNewestFirst)
  let bytes = 0
  let runs = 0
  ...
  await Promise.all(expired.map((path) => rm(path, { force: true })))
}
...
async function retainGlobalLogs(
  ...
  let directories
  try {
    directories = (await readdir(logsRoot, { withFileTypes: true })).filter((entry) =>
      entry.isDirectory(),
    )
  } catch (error) {
    if (hasCode(error, 'ENOENT')) return
  ...
  for (const file of files.toSorted(compareOldestFirst)) {
    if (bytes <= retention.maximumCompletedBytesGlobally) break
    expired.push(file.path)
    bytes -= file.size
  }
  await Promise.all(expired.map((path) => rm(path, { force: true })))
...
async function listCompletedLogs(directory: string): Promise<readonly RetainedLogFile[]> {
  let entries: string[]
  try {
...
async function removeStaleActiveLogs(
  ...
  let entries: string[]
  try {
    entries = (await readdir(directory)).filter((entry) => entry.endsWith('.active.jsonl'))
  } catch (error) {
    if (hasCode(error, 'ENOENT')) return
    throw error
  }
...
function activeLogProcessId(name: string): number | null {
  const match = /T\d{6}Z-(\d+)-[\da-f-]+\.active\.jsonl$/u.exec(name)
...
function compareNewestFirst(left: RetainedLogFile, right: RetainedLogFile): number {
  return right.modifiedAt - left.modifiedAt || right.path.localeCompare(left.path)
}

function compareOldestFirst(left: RetainedLogFile, right: RetainedLogFile): number {
  return left.modifiedAt - right.modifiedAt || left.path.localeCompare(right.path)
}

async function readPreviousState(path: string): Promise<RepoMirrorRunState | null> {
  try {
...
function isInvocationSource(value: unknown): value is RepoMirrorInvocationSource {
  return Object.values(RepoMirrorInvocationSource).some((source) => source === value)
}

function isLastRunStatus(value: unknown): value is RepoMirrorLastRunStatus {
  return Object.values(RepoMirrorLastRunStatus).some((status) => status === value)
}
