...
import {
  type IGetRepoMirrorLogsService,
  IRepoMirrorConfigurationService,
  IRepoMirrorLoggerService,
  IRepoMirrorStoreService,
  type FollowRepoMirrorLogsInput,
  type GetRepoMirrorLogsInput,
} from '../../../contract/index'
...
export class GetRepoMirrorLogsService implements IGetRepoMirrorLogsService {
  ...
  async *follow(input: FollowRepoMirrorLogsInput): AsyncGenerator<string, void> {
    await this.#assertExists(input.name)
    ...
    while (!input.signal.aborted) {
      ...
      const current = [...(await this.logger.readLines(input.name, input.lines))]
