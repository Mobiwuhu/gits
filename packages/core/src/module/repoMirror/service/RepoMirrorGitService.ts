...
import {
  ...
  type IRepoMirrorGitService,
  type RepoMirrorDefinition,
  type RepoMirrorGitOperation,
  type RepoMirrorHealth,
} from '../../../contract/index'
...
export class RepoMirrorGitService implements IRepoMirrorGitService {
  ...
  async inspect(definition: RepoMirrorDefinition, path: string): Promise<RepoMirrorHealth> {
    ...
    return {
      issues,
      state:
        issues.length === 0 ? RepoMirrorRepositoryState.Ready : RepoMirrorRepositoryState.Invalid,
    }
  }
  ...
  private runRead(path: string, args: readonly string[]): Promise<GitCommandResult> {
