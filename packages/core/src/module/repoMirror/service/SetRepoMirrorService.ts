...
export class SetRepoMirrorService implements ISetRepoMirrorService {
  ...
  async #setOne(
    ...
    try {
      ...
      const identities = await Promise.all(urls.map((url) => resolveRepoMirrorUrl(this.git, url)))
      ...
      if (!changed) return this.view.create(current, RepoMirrorAction.Unchanged)
      ...
      if (current.urls[0] !== updated.urls[0]) {
        ...
        if (!changedRemote.ok) {
          return this.view.create(current, RepoMirrorAction.Failed, false, {
            error: commandError(
      ...
      if (input.schedule !== undefined) {
        ...
        if (
          ...
          return this.view.create(updated, RepoMirrorAction.Failed, false, {
            error: commandError(
      ...
      return this.view.create(updated, RepoMirrorAction.Updated)
    } finally {
