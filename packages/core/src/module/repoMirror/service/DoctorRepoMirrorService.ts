...
export class DoctorRepoMirrorService implements IDoctorRepoMirrorService {
  ...
  async #doctorOne(
    ...
    if (input.remote) {
      const checked = await this.git.probeRemote(
        definition.urls[0],
  ...
  async #repair(
    ...
    if (releaseMirror === null) {
      return 'Mirror is being used by another process.'
    }
    const path = this.view.mirrorPath(definition.name)
    try {
      ...
      const temporaryRoot = await mkdtemp(
        join(this.paths.temporary, `${definition.name}-repair-`)
      )
      ...
      try {
        ...
        if (!cloned.ok) {
          return operationMessage(cloned.commands)
        }
        ...
        if (await pathExists(path)) {
          await moveToTrash(path, definition.name, this.paths)
        }
