...
export class RemoveRepoMirrorService implements IRemoveRepoMirrorService {
  ...
  async execute(input: RemoveRepoMirrorsInput): Promise<RepoMirrorCommandOutput> {
  ...
  async #removeOne(
    ...
    try {
      if ((await pathExists(mirrorPath)) && !(await this.git.isManagedMirror(mirrorPath))) {
      ...
      if (input.detachDependents) {
        const estimatedBytes =
          (await this.fileSystem.directorySize(resolve(mirrorPath, 'objects'))) ?? 0
      ...
      return this.view.create(definition, RepoMirrorAction.Removed, false, {
