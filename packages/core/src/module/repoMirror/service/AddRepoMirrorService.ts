...
import { createAutomaticSchedule, parseScheduleOption } from './repoMirrorSchedule'
...
export class AddRepoMirrorService implements IAddRepoMirrorService {
  ...
  async #plan(
    ...
    const resolved = await Promise.all(input.urls.map((url) => resolveRepoMirrorUrl(this.git, url)))
    ...
    const aliases = await Promise.all(
      input.aliases.map((url) => resolveRepoMirrorUrl(this.git, url)),
    )
    const configuredIdentities = await this.configuration.identities(configuration)
  ...
  async #addOne(plan: AddPlan, input: AddRepoMirrorsInput): Promise<RepoMirrorView> {
    ...
    try {
      if (plan.existing) return this.#updateExisting(plan)
      ...
      try {
        if (await pathExists(finalPath)) {
          return this.view.create(plan.definition, RepoMirrorAction.Failed, false, {
        ...
        return this.view.create(plan.definition, RepoMirrorAction.Created)
      } finally {
  ...
  async #updateExisting(plan: AddPlan): Promise<RepoMirrorView> {
    ...
    try {
      ...
      if (changed) await this.store.save(this.configuration.replace(configuration, desired))
      await releaseConfig()
      configReleased = true
      if (changed && plan.explicitSchedule) {
        ...
        if (
          ...
          return this.view.create(desired, RepoMirrorAction.Failed, false, {
            error: commandError(
              'scheduler-apply-failed',
              schedule.message ?? 'Native scheduler is unavailable.',
            ),
